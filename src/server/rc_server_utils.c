...
int setup_client_resources(struct conn_info *connInfo) {
    int ret = -1;
    check(!connInfo->rc_connection->cm_client_id, -EINVAL, "Client id is still NULL\n", -EINVAL);
    ...
    check(!connInfo->rc_connection->pd, -errno, "Failed to allocate a protection domain errno: %d\n",
          -errno);

    pr_debug("A new protection domain is allocated at %p \n", connInfo->rc_connection->pd);
     ...
     */
    connInfo->rc_connection->io_completion_channel = ibv_create_comp_channel(
            connInfo->rc_connection->cm_client_id->verbs);
    check(!connInfo->rc_connection->io_completion_channel, -errno,
          "Failed to create an I/O completion event channel, %d\n",
          -errno);

    pr_debug("An I/O completion event channel is created at %p \n",
             connInfo->rc_connection->io_completion_channel);
    ...
    check(!connInfo->rc_connection->cq, -errno, "Failed to create a completion queue (cq), errno: %d\n",
          -errno);

    pr_debug("Completion queue (CQ) is created at %p with %d elements \n",
             connInfo->rc_connection->cq, connInfo->rc_connection->cq->cqe);
    ...
    pr_debug("Client QP created at %p\n", connInfo->rc_connection->client_qp);
    return ret;
}
...
int init_rc_server(struct conn_info *connInfo) {
    ...
    check(!connInfo->rc_connection->cm_event_channel, -errno,
     ...
     */
    ret = rdma_create_id(connInfo->rc_connection->cm_event_channel, &connInfo->rc_connection->cm_server_id, NULL,
                         RDMA_PS_TCP);
...
int rc_accept_new_connection(struct rc_server_info *listening) {
    ...
    int ret = -1;
    check(!listening->cm_client_id || !listening->cm_client_id, -EINVAL,
    ...
    check(!listening->request_mr, -ENOMEM, "Failed to register client attr buffer\n", -ENOMEM);
